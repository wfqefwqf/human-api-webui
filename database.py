...
def _get_conn():
    if not hasattr(_lock, "conn") or _lock.conn is None:
        _lock.conn = sqlite3.connect(DB_PATH, check_same_thread=False)
...
def _get_shared_conn():
    if not hasattr(_lock, "shared_conn") or _lock.shared_conn is None:
        _lock.shared_conn = sqlite3.connect(DB_PATH, check_same_thread=False)
        _lock.shared_conn.row_factory = sqlite3.Row
        _lock.shared_conn.execute("PRAGMA journal_mode=WAL")
        _lock.shared_conn.execute("PRAGMA foreign_keys=ON")
    return _lock.shared_conn
